...
function exportCSV(data: Pendaftar[]) {
  const headers = ["No. Pendaftaran", "Nama Lengkap", "Email", "No. HP", "Asal S1", "IPK", "Jalur Masuk", "Status", "Tanggal Daftar"];
  ...
  a.href = url;
  a.download = `data-pendaftar-s2-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
  URL.revokeObjectURL(url);
}

export default function PendaftarClient({ data }: { data: Pendaftar[] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm" style={{ color: "#2d5a7a" }}>
            Seluruh data pendaftar Program Magister Ilmu Kelautan
          </p>
        </div>
        <button
      ...
      <div className="flex gap-3 flex-wrap">
        {[
          { label: "Total", value: data.length, color: "#38bdf8", bg: "rgba(14,165,233,0.12)" },
          { label: "Pending", value: data.filter((d) => d.status === "pending").length, color: "#fbbf24", bg: "rgba(245,158,11,0.12)" },
          { label: "Lolos Adm.", value: data.filter((d) => d.status === "lolos_administrasi").length, color: "#22d3ee", bg: "rgba(6,182,212,0.12)" },
          { label: "Diterima", value: data.filter((d) => d.status === "diterima").length, color: "#34d399", bg: "rgba(16,185,129,0.12)" },
          { label: "Tidak Lolos", value: data.filter((d) => d.status === "tidak_lolos").length, color: "#f87171", bg: "rgba(239,68,68,0.12)" },
        ].map(({ label, value, color, bg }) => (
          <div key={label} className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold"
        ...
        <div className="px-5 py-4 border-b space-y-3" style={{ borderColor: BORDER }}>
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm" style={{ color: "#e0f2fe" }}>Daftar Pendaftar</span>
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="space-y-0.5">
                            <div className="text-xs" style={{ color: "#4a7fa5" }}>{row.email}</div>
                            {row.nomor_hp && (
                              <div className="flex items-center gap-1 text-xs" style={{ color: "#2d5a7a" }}>
                                <Phone size={10} />
                                {row.nomor_hp}
                              </div>
                            )}
                        ...
                        <td className="px-4 py-3 max-w-[130px] truncate text-xs" style={{ color: "#4a7fa5" }}>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t" style={{ borderColor: BORDER }}>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((p, i) =>
                      p === "..." ? (
                        <span key={`e-${i}`} className="w-8 h-8 flex items-center justify-center text-xs" style={{ color: "#2d5a7a" }}>…</span>
                      ) : (
                        <button
                          key={p}
